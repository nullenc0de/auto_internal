...
cat domain_users.json |grep -A1 sAMAccountName | cut -d '"' -f2 |grep -v "sAMAccountName" |sort -u > logs/AD_users.txt
echo 
echo $(cat logs/AD_users.txt |wc -l) "Users found in logs/AD_users.txt"
